...
const DepartmentPerformanceDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDepartmentData = async () => {
      ...
      try {
        ...
        const response = await api.get(`/stats/department-performance/${id}`);

        const departmentId: string = id.toString();
        const data = response.data[departmentId];
  ...
  const chartData = departmentData
    ? {
        ...
        datasets: [
          {
            label: "Average Score",
            data: departmentData.competencies.map((comp) => comp.average_score),
          ...
          },
          {
            ...
            data: departmentData.competencies.map(
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: departmentData
          ? `${departmentData.department_name} Department Competency Scores`
          ...
          >
            ...
            >
              ...
              <p style={{ fontSize: "24px", fontWeight: "bold" }}>
                {departmentData.overall_average_score.toFixed(2)}
              </p>
            </div>
            <div
              style={{
            ...
            >
              ...
              <p style={{ fontSize: "24px", fontWeight: "bold" }}>
                {departmentData.overall_fulfillment_rate.toFixed(2)}%
              </p>
            </div>
          </div>
            ...
            {departmentData.competencies.length > 0 ? (
              <div style={{ overflowX: "auto" }}>
                ...
                >
                  ...
                  <tbody>
                    {departmentData.competencies.map((comp) => (
                      <tr
                        key={comp.competency_code}
                        style={{
