...
interface ManinAssignedCompetency {
  competency_code: string;
  required_score: number;
}

type AssignedCompetencyType = {
  competency_code: string;
  required_score: number;
};
type RoleCompetencyType = {
  competency_code: string;
  role_competency_required_score: number;
};

const EmpCompetencyAssignment: React.FC = () => {
  const { employeeNumber } = useParams<{ employeeNumber: string }>();
  const navigate = useNavigate();
  const [isExpanded, setIsExpanded] = useState(false);
  const [employeeName, setEmployeeName] = useState<string>("");

  const [MainAssignCompetencies, setMainAssignCompetencies] = useState<
    ManinAssignedCompetency[]
  >([]);
  ...
  const fetchAllData = async () => {
    try {
      const [compsRes, empCompsRes, EmpDetailsRes] = await Promise.all([
        api.get("/competency"),
        api.get<AssignedCompetencyType[]>(
          `/assign-employees/${employeeNumber}/competencies`
        ),
        api.get(`/employees/${employeeNumber}`),
      ]);
      const response = await api.get<RoleCompetencyType[]>(
        `/roles/${EmpDetailsRes.data.role_id}/competencies/detailed`
      );
      setCompetencies(compsRes.data);
      setAssignedCompetencies(
        empCompsRes.data.filter(
          (c) =>
            !response.data.some((a) => a.competency_code == c.competency_code)
        )
      );
      setMainAssignCompetencies(empCompsRes.data);

      if (EmpDetailsRes.data?.employee_name) {
        setEmployeeName(EmpDetailsRes.data.employee_name);
      } else {
        setEmployeeName(employeeNumber ? employeeNumber : "None");
      }
  ...
  const selectAll = (type: "available" | "assigned") => {
    if (type === "available") {
      const availableCodes = competencies
        .filter(
          (c) =>
            !assignedCompetencies.some(
              (ac) => ac.competency_code === c.competency_code
            )
        )
      ...
      setSelectedCompetencies(
        assignedCompetencies.map((c) => c.competency_code)
      );
    }
  };
  ...
  const handleAssign = async () => {
    ...
    if (
      window.confirm(
        "This will also reflect on employee competencies are you sure"
      )
    ) {
  ...
  const handleRemove = async () => {
    ...
    if (
      ...
      try {
        await api.delete(`/assign-employees/${employeeNumber}/competencies`, {
    ...
    if (window.confirm("This will also reflect on employee competencies are you sure")) {
      try {
        const payload = assignedCompetencies.map(comp => ({
          ...
          required_score: competencyScores[comp.competency_code] 
            ? parseInt(competencyScores[comp.competency_code]) 
            : comp.required_score
        }));
  
        await api.put(
          `/assign-employees/${employeeNumber}/competencies/scores`,
          payload
        );
  ...
  // Update the toggle function to initialize scores as strings
  const toggleScoreEditing = () => {
    ...
    if (!isEditingScores) {
      ...
      assignedCompetencies.forEach(comp => {
        currentScores[comp.competency_code] = comp.required_score.toString();
      });
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-xl mt-20 text-gray-600">Loading...</div>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 font-sans mt-20">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <p className="text-medium  text-gray-700">Name: {employeeName}</p>
      ...
      {activeTab === "assign" ? (
        <div className="flex flex-col gap-6">
          ...
          {/* total employee competencies */}
          <div className="bg-green-50 rounded-2xl shadow border border-green-200  transition-all">
            <div className="flex justify-between items-center px-6 py-4 border-b border-green-200">
              ...
              <button
                ...
                className="flex items-center text-green-700 hover:text-green-900 font-medium focus:outline-none"
              >
                {isExpanded ? (
                  <>
                    Collapse <ChevronUp className="ml-2 w-5 h-5" />
                  </>
                ) : (
                  <>
                    Expand <ChevronDown className="ml-2 w-5 h-5" />
                  </>
                )}
              </button>
            </div>
            ...
            {isExpanded && (
              <div className="transition-all duration-300 ease-in-out">
                {MainAssignCompetencies.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full table-auto">
                      <thead className="bg-green-100">
                        <tr>
                          <th className="pl-5 w-32 p-4 text-left font-semibold text-gray-700">
                            Code
                          </th>
                          ...
                          <th className="w-32 p-4 text-center font-semibold text-gray-700 whitespace-nowrap">
                            Required Score
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-green-100">
                        {MainAssignCompetencies.map((comp) => {
                          const nameData = competencies.find(
                            (c) => c.competency_code === comp.competency_code
                          );
                          return (
                            <tr
                              key={`score-${comp.competency_code}`}
                              className="hover:bg-green-100 transition"
                            >
                              <td className="p-3 font-semibold text-gray-700">
                                {nameData?.competency_code}
                              </td>
                              <td className="p-3 text-gray-800">
                                {nameData?.competency_name}
                              </td>
                              <td className="p-3 text-gray-600">
                                {nameData?.competency_description}
                              </td>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-64">
            ...
            {competencies.every((all) =>
              assignedCompetencies.some(
                (ac) => ac.competency_code === all.competency_code
              )
            ) ? (
              <div className="py-8 text-center text-gray-500 italic">
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="w-32 p-4 text-left font-semibold text-gray-600">
                        Code
                      </th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {competencies
                      .filter(
                        (c) =>
                          !MainAssignCompetencies.some(
                            (ac) => ac.competency_code === c.competency_code
                          )
                      )
                      .map((comp) => (
                        <tr
                          key={`available-${comp.competency_code}`}
                        ...
                        >
                          <td className="p-3">
                            <input
                              ...
                              // disabled={true}
                              checked={selectedCompetencies.includes(
                                comp.competency_code
                              )}
                              onChange={() =>
                                toggleCompetency(comp.competency_code)
                              }
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-64">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-700">
                Assigned Competencies For this employee only
              </h3>
              <div className="flex gap-3">
                <button
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="w-32 p-4 text-left font-semibold text-gray-600 whitespace-nowrap">
                        Required Score
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {competencies
                      ...
                      .map((comp) => {
                        ...
                        return (
                          <tr
                            key={`assigned-${comp.competency_code}`}
                            className="hover:bg-gray-50"
                          >
                            <td className="p-3">
                              <input
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 min-h-64">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-700">
                Set Required Scores for Competencies
              </h3>
              <div className="flex gap-3">
                <button
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="w-40 p-4 text-left font-semibold text-gray-600">
                        Required Score
                      </th>
                    </tr>
                  </thead>
  ...
  value={
    isEditingScores
      ? competencyScores[comp.competency_code] !== undefined
        ...
        : assignedComp?.required_score.toString() || ""
      : assignedComp?.required_score.toString() || ""
  }
  onChange={(e) =>
    handleScoreChange(
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  assignedCompetencies.forEach((comp) => {
                    originalScores[comp.competency_code] = comp.required_score.toString();
                  });
...
export default EmpCompetencyAssignment;
