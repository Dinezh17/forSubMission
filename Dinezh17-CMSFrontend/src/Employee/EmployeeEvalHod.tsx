...
const EmployeeEvaluation: React.FC = () => {
  ...
  return (
    <div className="mt-20 p-5 mx-10  bg-white rounded-lg shadow-lg ">
      <h2 className="text-2xl font-semibold text-gray-800 mb-4">Employee Evaluation</h2>
        ...
        <table className="w-full table-auto border-collapse">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="p-4 text-left text-sm font-semibold text-gray-600">Job</th>
              <th className="p-4 text-left text-sm font-semibold text-gray-600">Reporting To</th>
              ...
              <th className="p-4 text-left text-sm font-semibold text-gray-600">Last Evaluated</th>
          ...
          <tbody>
            {filteredEmployees.length > 0 ? (
              filteredEmployees.map((employee) => {
                ...
                const reportingName =
                  employees.find((d) => d.employee_number.toString() === employee.reporting_to)?.employee_name ||
                  "N/A";
                ...
                return (
                  <tr key={employee.employee_number} className="transition-colors hover:bg-gray-50">
                    <td className="p-4 text-sm text-gray-700">
                      <div className="font-semibold text-gray-800">{employee.employee_name}</div>
                      <div className="text-xs text-gray-500 mt-1">{employee.employee_number}</div>
                    </td>
                    <td className="p-4 text-sm text-gray-700">
                      <div>{employee.job_name}</div>
                      <div className="text-xs text-gray-500 mt-1">{employee.job_code}</div>
                    </td>
                    <td className="p-4 text-sm text-gray-700">{reportingName}</td>
                    <td className="p-4 text-sm text-gray-700">{deptName}</td>
                    <td className="p-4 text-sm text-gray-700">{roleName}</td>
                    <td className="p-4 text-sm text-gray-700">{roleCategory}</td>
                    <td className="p-4 text-sm text-gray-700">
                      <span
                        className={`inline-block px-4 py-2 rounded-full text-xs font-semibold ${
                          employee.evaluation_status === null || employee.evaluation_status === undefined
