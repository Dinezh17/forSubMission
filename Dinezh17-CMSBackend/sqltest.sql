-- insert into departments (id , name ) VALUES (100 , 'dummy');

-- INSERT INTO roles (id, role_code, role_name, role_category) VALUES
-- (100, 'dummy', 'dummy', 'dummy');


-- UPDATE users set role = "HR" WHERE employee_number ="test102";

-- delete from employees where role_id="104"
-- UPDATE users set role = "Manager" WHERE employee_number = "hod"
-- INSERT into role_job values("dummy100","dummy",100,True)

-- INSERT INTO employees 
-- VALUES
--     ('100000000', 'dummy', "dummy100",null,100,100, 'Not sent by anyone', NULL, 'Not evaluated', NULL);
  insert into department_roles values(2,100,100)
  